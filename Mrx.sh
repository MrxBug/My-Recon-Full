...
naabu -list "$folder/subdomains.txt" -top-ports 100 -c 50 -o "$folder/PortScan.txt"
...
cat "$folder/live_subdomains.txt" | gau --threads 10 --blacklist "$excluded_extentions" --o "$folder/EndpointsGau.txt"
...
# cve,osint,tech
echo -e "\e[32mExecutando cve,osint,tech Vulnerabilit\e[0m"
cat "$folder/live_subdomains.txt" | nuclei -tags cve,osint,tech -o "$folder/CvOsTecvul.txt"
